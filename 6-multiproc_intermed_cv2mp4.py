...
def process_video(group_number):
    ...
    proc_frames=0
    out = cv2.VideoWriter('{}.mp4'.format(group_number),cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height)) 
...
t2 = time.time()
sp.Popen('ffmpeg -y -loglevel warning -f concat -safe 0 -i intermediate_files.txt -c copy output_method6.mp4', shell=True).wait()
t3 = time.time()
sp.Popen('ffmpeg -y -loglevel warning -f concat -safe 0 -i intermediate_files.txt -c copy -preset ultrafast output_method6.mp4', shell=True).wait()
t4 = time.time()
